...
6. enums — 枚举层
   订单状态、演唱会状态、场次状态在代码中全部以 0/1/2/3/4 这样的魔术数字出现，可读性差且容易写错。 这里定义了对应的 Java 枚举类型，提高了代码的可读性和可维护性。
7. exception — 异常层
   自定义异常类，统一异常处理，返回统一格式的错误信息。项目没有 @RestControllerAdvice，所有业务异常都用裸的 RuntimeException 抛出，Controller 用 catch(RuntimeException) 笼统地接住——这意味着任何意外的空指针异常、类型转换异常都会被当作业务错误返回给前端，可能泄露内部堆栈信息。
因此建立了一个自定义异常体系（BusinessException、NotFoundException 等）和全局异常处理器。
8. mapper — 持久层
